...
- For Intervals planned-workout repeat blocks, verify both acceptance and provider interpretation. A payload can be accepted but still semantically wrong if a titled repeat like `Main Set 4x` is parsed as a label instead of a repeat; lock the provider-specific `workout_doc` grammar with tests that preserve total-duration semantics.
