...
- **Forward-load horizon + dated load:** tool supports `horizon_days` (1..=45); out-of-window dated entries must note why they were ignored; today/past race-named load updates baseline via `baseline_applied_load` so evidence can cite `baseline_with_estimated_race_load`.
- **Post-deploy Fix 8 replay (op `i188477359`):** compare score SEQUENCE vs v1.0.147 (`7,7,7,8,7,7,8,7,7,7`) and v1.0.148 (`7,7,7,7,7,8,7,7,7,7`). Critiques must not ask for “through 25 October” or “labeled estimated race TSS included” when evidence already shows extended `days=` plus `baseline_with_estimated_race_load` / `race_tss=`.
- **Fix 9 TT discipline mapping:** `RACING_STRATEGIST_OPERATIONAL_GUIDELINES` is generator-only (evaluator explicitly excludes it). Put disc→session in ops + a BASE precedence override for `timetrial`/≤~30 min so “repeatability-first” cannot win; bind sustained blocks in `plan`, not only `description`. Cover `def_disc` as well as `rc.disc`. Pre-deploy baseline for `i188477359`: SEQUENCE `8,7,7,7,8,7,8,7,8,7` with 2–4m “Time-Trial Durability” — post-deploy expect continuous ≥15m @90–95% and no “2x10 / 3x8” critiques.
- **Fix 10 completed race + discipline gate:** pass finished race TSS via `completed_race` (not dated syntax); compact with `source=measured|estimated`. Quality loop must reject drafts missing the discipline-required continuous/repeatability session so short-block 8s cannot replace continuous 7s. Post-deploy SEQUENCE vs `7,8,7,7,7,8,7,8,7,8`.
- **Fix 11 first-draft gate + long-window clip:** `replan_gate_gap` alone is not enough — attempt 1 never entered regenerate, so a gated score-9 short TT still won best-of. Apply `missing_discipline_requirement` on every attempt before `is_better`/`Accepted`; keep a score-only `fallback` when every draft fails the gate. Plans longer than 14 days clip to the earliest window (`clip_days_to_window`) instead of `Validation` abort. Post-deploy replay vs SEQUENCE `9,8,7,7`: expect continuous ≥780s @88–96% on shipped snapshot and no `shipping best draft so far` from overlong windows.
