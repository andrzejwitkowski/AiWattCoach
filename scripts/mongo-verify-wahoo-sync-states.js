...
Optional environment variables:
  ...
  MIGRATION_USER_ID               Optional. Limit verification to one user.

Exit code:
  0 when every legacy Wahoo sync row has a matching migrated target row.
  1 when rows are missing, mismatched, or duplicate legacy identifiers exist.
*/

(function verifyWahooSyncStates() {
  ...
  const summary = {
    ...
    missing: 0,
    mismatched: 0,
    duplicatePlanIds: duplicateSummary.planIdDuplicates.length,
    duplicateWorkoutTokens: duplicateSummary.workoutTokenDuplicates.length,
    extraTargetRows: 0,
    missingSamples: [],
    mismatchSamples: [],
    extraSamples: [],
  ...
  while (cursor.hasNext()) {
    ...
    const targetFilter = buildTargetFilter(expected);
    const actual = target.findOne(targetFilter);

    summary.scanned += 1;
    sourceKeys.push(buildKey(targetFilter));

    if (!actual) {
      summary.missing += 1;
      pushSample(summary.missingSamples, {
        user_id: expected.user_id,
        canonical_entity_id: expected.canonical_entity_id,
      });
      continue;
    }
  ...
  if (config.userId) {
    targetFilter.user_id = config.userId;
  }

  const targetCursor = target.find(targetFilter).sort({ user_id: 1, canonical_entity_id: 1 });
  while (targetCursor.hasNext()) {
    const migrated = targetCursor.next();
    const key = buildKey({
    ...
    summary.extraTargetRows += 1;
    pushSample(summary.extraSamples, {
      user_id: migrated.user_id,
      canonical_entity_id: migrated.canonical_entity_id,
    });
  }
  ...
  if (
    ...
    summary.duplicatePlanIds > 0 ||
    summary.duplicateWorkoutTokens > 0
  ) {
...
function buildKey(targetFilter) {
  return [
    targetFilter.user_id,
    targetFilter.provider,
    targetFilter.canonical_entity_kind,
    targetFilter.canonical_entity_id,
  ].join("|");
}
