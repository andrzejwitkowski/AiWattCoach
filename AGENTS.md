...
- On Windows in this environment, check the Obsidian vault under `E:\obsidian\vault\opencode`.
...
- Spaghetti code is not acceptable in this repo. Do not ship or leave behind giant mixed-responsibility files that combine document shapes, mapping, query helpers, repository logic, and tests in one place just because they still compile. Split them before calling the work done.
