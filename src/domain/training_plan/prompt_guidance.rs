...
mod tests {
    ...
    #[test]
    fn planning_guidelines_override_repeatability_for_timetrial() {
        let guidelines = training_plan_planning_guidelines(false, 14);
        assert!(guidelines.contains(
            "When rc.disc or def_disc is timetrial, or the target event's estimated duration is under ~30 minutes, the repeatability-first rule does NOT apply: sustained near-threshold work takes priority over stochastic repeatability."
        ));
        assert!(guidelines.contains("Time-Trial Threshold"));
    }
