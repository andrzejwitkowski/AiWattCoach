...
impl LlmTool for GetSelectedWorkoutById {
    ...
    fn definition(&self) -> LlmToolDefinition {
        LlmToolDefinition {
            ...
            description: "Get detailed workout data for a specific frontend-visible workout id. Use this for the currently selected workout instead of inferring a date from nearby history. Returns completed workouts with statistics and aligned_intervals when a linked planned workout exists, heartrate at 1-second resolution when present, plus AI conversation history and related plan/race context for that workout date.".to_string(),
    ...
    fn prompt_guidance(&self) -> Option<&'static str> {
        Some(
            "use for the currently selected workout when you need exact workout details or aligned_intervals without guessing the date",
        )
    }
