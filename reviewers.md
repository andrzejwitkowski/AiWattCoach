...
- Prevention: when hidden provider transcript is the canonical continuity layer, treat it as an append-only ordered history unless there is an explicit truncation rule. Replay logic should be resilient to transport/storage normalization like trimmed whitespace, and crash recovery for provider checkpoints must always converge to either `Completed` or `Failed` rather than leaving durable operations indefinitely pending when no final assistant text exists.

### 2026-05-02 | Copilot internal review | PR #177 fresh tool-only response and stale REST reply follow-up

- Problem: even after fixing recovery for tool-only transcripts, the live fresh-response path still left durable reply operations in `Pending` when the provider returned assistant tool calls without a final text message. The code checkpointed the provider response and public tool bubbles, then returned `InvalidResponse` while skipping the usual `mark_failed(...)` persistence step. Separately, the calendar hook still let a late REST `sendCalendarCoachMessage(...)` response for conversation A overwrite newly opened conversation B, because the stale-conversation guard only protected websocket events and not locally applied async REST responses.
- Fix: in both coach flows, required final assistant text now trims and validates the response after checkpointing; if no final text exists, the code persists the operation as `Failed` before returning `InvalidResponse`. In `workout_summary`, the checkpoint helper now upserts the operation before mirroring `hidden_transcript` onto the summary so the durable operation record is written first. In `useCalendarCoachChat.ts`, `sendMessage()` now checks `currentConversationIdRef` before sending over websocket, before applying REST send success, and before applying or clearing state from 404 reload fallback. Added focused regressions for fresh tool-only responses becoming `Failed` in both coach flows and for ignoring stale REST send responses after switching conversations.
- Prevention: whenever a workflow checkpoints provider output before validating the final response shape, make the validation failure path persist a terminal `Failed` state, not just return an error. For frontend conversation state, apply the same stale-request guard to local async REST callbacks that you already use for websocket events; otherwise switching entities mid-flight can still reintroduce old state.

### 2026-05-02 | Copilot internal review | PR #177 final stale-state and recovery hardening

- Problem: the final review pass found two more stale-state gaps in the calendar hook and one recovery hardening gap in workout-summary replay. `loadConversation()` could still apply an old initial load after a newer conversation had already taken over local state, and `ensureConversation()` could still apply an older implicit create result after the user had opened a different fresh conversation. Separately, workout-summary recovery checked existing public tool bubbles with `get_message_by_id(...).await.is_ok()`, which treated real repository failures the same as missing messages and could silently duplicate tool bubbles or hide storage errors.
- Fix: added a shared `isCurrentConversation(...)` guard in `useCalendarCoachChat.ts` and applied it to initial load, implicit conversation creation, send success, send fallback reload, and stale send-error handling; added focused frontend regressions for stale load responses and stale implicit-create responses after switching conversations; and changed workout-summary recovery to distinguish `NotFound` from real repository errors when probing for existing tool messages, propagating non-`NotFound` failures instead of treating them as safe misses.
- Prevention: once a stale-entity guard exists in a conversation hook, audit every async state write path against it, not only websocket events and the latest send path. In recovery code, never collapse repository errors into presence checks with boolean helpers like `.is_ok()` when the difference between "not found" and "storage failure" affects idempotency.

### 2026-05-02 | Copilot internal review | PR #177 hidden transcript merge and dedup

- Problem: two hidden-transcript durability gaps. First, `replace_hidden_transcript` overwrote the summary/conversation transcript with only the current operation's entries, losing older assistant turns from the shared aggregate after each checkpoint. Second, crash recovery blindly extended the reclaimed operation's transcript into the summary, duplicating entries already present from a pre-crash merge that succeeded before the crash.
- Fix: changed fresh checkpoints in both flows to merge (not replace) the operation's entries into the existing aggregate transcript; changed recovery paths to use deduplicated addition (`contains` check before push) instead of blind extend.
- Prevention: treat aggregate transcript as append-only canonical history; only add new entries not yet present. In recovery, always check for idempotency of transcript reconstruction.
